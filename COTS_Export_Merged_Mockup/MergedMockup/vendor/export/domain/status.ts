...
const TONE_MAP: Record<string, StatusTone> = {
  ...
  ready_finished: "ok",
  reserved: "accent",
  /* tags / artwork specification (v2.0 §6.5), two-party since 9 September 2026.
     `confirmed` is deliberately absent: this map is keyed by the bare state string across every
     domain, and a document is already `confirmed` above with tone `info`. The tag card passes
     its own tone for that state rather than a second key overwriting the document's. */
  sent: "info",
  amended: "warn",
  reissued: "warn",
...
export const TAG_SPECIFICATION_TRANSITIONS: Record<TagSpecificationState, TagSpecificationState[]> = {
  not_started: ["sent"],
  /* With Dubai: they confirm it, or return it with an amendment. */
  sent: ["confirmed", "amended"],
  /* Back with Origin, who sends the amended tag. */
  amended: ["sent"],
  /* Not terminal — reprocessing changes the packaging and the tags (v2.0 §6.5 exception). */
  confirmed: ["reissued"],
  /* And a re-issued specification goes round the loop again rather than being confirmed on the
     spot: the tag has changed, so Origin sends the new one and Dubai answers it. */
  reissued: ["sent"],
};
