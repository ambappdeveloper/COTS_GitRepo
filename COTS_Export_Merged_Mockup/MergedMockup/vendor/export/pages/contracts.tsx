...
const CONTRACT_TABS = [
  "summary",
  "details",
  /** v2.0 phase 04 — the four cross-functional fulfilment confirmations. */
  "review",
...
export function ContractDetail() {
  ...
  const ctPlans = (plans.data ?? []).filter((p) => p.contractId === c.id);
  const ctCargo = (cargo.data ?? []).filter((cr) => ctPlans.some((p) => p.id === cr.executionPlanId));
  ...
  const balance = quantityBalance(c, {
    plannedMt: ctPlans.reduce((a, p) => a + p.plannedQuantityMt, 0),
    readyMt,
    shippedMt,
  });
              ...
              <SummaryCard title="Quantity & allocation">
                ...
                <FieldGrid
                  ...
                  fields={[
                    ...
                    {
                      label: "Planned across lots",
                      value: formatMt(balance.plannedMt),
                      behaviour: "calculated",
                    },
            ...
            <div className="grid-2">
              ...
              >
                <Banner tone="info" title="Proposed step — no legacy screen exists">
                  Workshop requirement: on contract creation, quality, execution, processing and FP&amp;A are
                  alerted, review the terms and record their feedback in COTS.
                </Banner>
        ...
        {activeTab === "planning" ? (
          <>
            ...
            {ctPlans.length === 0 ? (
              ...
              <div className="stack">
                {ctPlans.map((p) => {
                  const cr = ctCargo.find((x) => x.executionPlanId === p.id);
                  return (
                    <CollapsibleSection
                      key={p.id}
                      title={`${p.planningNo} — ${formatMt(p.plannedQuantityMt)}`}
                      defaultOpen={ctPlans.length <= 2}
                      indicator={
                        <>
                          <StatusChip tone={toneFor(p.status)} label={humanise(p.status)} size="sm" />
                          {cr ? (
                            <StatusChip
                              tone={toneFor(cr.cargoStatus)}
                              label={humanise(cr.cargoStatus)}
                              size="sm"
                            />
                          ) : null}
                        </>
                    ...
                    >
                      <FieldGrid
                        fields={[
                          { label: "Planning number", value: p.planningNo, behaviour: "readonly" },
                          {
                            label: "Planned quantity",
                            value: formatMt(p.plannedQuantityMt),
                            behaviour: "required",
                          },
                          { label: "Port of loading", value: portName(p.portOfLoadingId) },
                          { label: "Shipper", value: p.shipperName },
                          { label: "Shipper on behalf of", value: p.shipperOnBehalfOf },
                          { label: "Bank", value: `${p.bank}${p.bankBranch ? ` · ${p.bankBranch}` : ""}` },
                          { label: "Seasonality", value: p.seasonality },
                          { label: "Cargo source", value: p.cargoSource, behaviour: "required" },
                          { label: "Export contract payment terms", value: p.exportContractPaymentTerms },
                          { label: "Urgency", value: humanise(p.urgency) },
                          { label: "Assigned to", value: p.assignedTo },
                          { label: "Receiving at facility", value: formatDate(p.receivingAtFacilityDate) },
                          { label: "To be shipped before", value: formatDate(p.toBeShippedBefore) },
                          {
                            label: "Raw quantity",
                            value: p.rawQuantityMt !== undefined ? formatMt(p.rawQuantityMt) : undefined,
                          },
                          {
                            label: "Finished quantity",
                            value:
                              p.finishedQuantityMt !== undefined ? formatMt(p.finishedQuantityMt) : undefined,
                          },
                          {
                            label: "Needs processing",
                            value:
                              p.needProcessingMt !== undefined ? formatMt(p.needProcessingMt) : undefined,
                          },
                        ]}
                      />
                      {cr ? (
                        <div style={{ marginTop: "1rem" }}>
                          <h4 className="small" style={{ fontWeight: 600 }}>
                            Cargo readiness &amp; stock allocation
                          </h4>
                          <FieldGrid
                            fields={[
                              { label: "Cargo status", value: humanise(cr.cargoStatus) },
                              { label: "Status date", value: formatDate(cr.statusDate) },
                              { label: "Quantity ready", value: formatMt(cr.qtyReadyMt) },
                              {
                                label: "Remaining to ready",
                                value: formatMt(Math.max(0, p.plannedQuantityMt - cr.qtyReadyMt)),
                                behaviour: "calculated",
                              },
                            ]}
                          />
 ...
 * reports it cannot fulfil the contract. The screen therefore captures the four
...
function ContractReviewTab({ c, onSaved }: { c: Contract; onSaved: () => void }) {
  ...
  return (
    <>
      ...
      <p className="page__intro">
        Phase 04 of workflow v2.0. On contract creation, Quality, the Execution team, the Processing team and
        FP&amp;A are alerted, each reviews the terms and each confirms it has the set-up in place to fulfil
        them. Four confirmations, one per function.
      </p>
      ...
      <div className="grid-3">
        <SummaryCard title="Confirmations" tone={confirmed === 4 ? "ok" : "accent"}>
          <QuantityDonut
        ...
        <SummaryCard title="Notification channel">
          <FieldGrid
            ...
            fields={[
              ...
              {
                label: "Same four teams in every country?",
                value: "Not stated",
      ...
      <CollapsibleSection title="The four fulfilment confirmations">
            ...
            <tbody>
              {c.reviewFeedback.map((f) => (
                <tr key={f.role}>
                  ...
                  <td className="small">{f.comment ?? "–"}</td>
                  <td>
        ...
        <ul className="doclist">
          <li>
            <span className="doclist__name">
              ...
              <span className="doclist__sub">Decision D-10 — Execution, Quality, Processing, FP&amp;A</span>
            </span>
          ...
          <li>
            <span className="doclist__name">
              Are the same four teams notified in every country?
...
function ContractQualityTab({ c, onSaved }: { c: Contract; onSaved: () => void }) {
  ...
  const [tagState, setTagState] = useState<TagSpecificationState>(spec?.state ?? "not_started");
  ...
  const [agreedBy, setAgreedBy] = useState(spec?.agreedBy ?? "");
  ...
  async function saveTags() {
    ...
    const tags = customTags
      ...
      .map((t) => t.trim())
      .filter(Boolean);
    if (tagOption === "buyer" && tagState !== "not_started" && tags.length === 0) {
      errs["tg-custom"] =
        "A buyer option lists the custom tag options the buyer requires (v2.0 §6.5 activity 3).";
    }
    if (tagState === "agreed" && !agreedBy.trim()) {
      errs["tg-by"] = "Say who agreed the specification.";
    }
    setTagErrors(errs);
    if (Object.keys(errs).length > 0) return;
    setSavingTags(true);
    const res = await api.setTagSpecification(c.id, {
      option: tagOption,
      state: tagState,
      customTags: tags,
      batchCodeOnTag: batchOnTag,
      agreedOn: tagState === "agreed" ? TODAY : spec?.agreedOn,
      agreedBy: agreedBy.trim() || undefined,
      reissuedAfterReprocessingOn: tagState === "reissued" ? TODAY : spec?.reissuedAfterReprocessingOn,
      note: spec?.note,
    });
    setSavingTags(false);
    if (!res.ok) {
    ...
    toast.push("ok", "Tags specification saved.");
    onSaved();
  }

  const allowedTagStates = nextStates(TAG_SPECIFICATION_TRANSITIONS, spec?.state ?? "not_started");
    ...
    <>
      ...
      <div className="grid-3">
        ...
        <SummaryCard
          ...
          tone={spec?.state === "agreed" ? "ok" : spec?.state === "reissued" ? "warn" : "accent"}
        >
          <FieldGrid
            columns={1}
            fields={[
              ...
              { label: "Agreed on", value: formatDate(spec?.agreedOn) },
              { label: "Agreed by", value: spec?.agreedBy },
              {
      ...
      <CollapsibleSection title="Tags specification flow (activity 3)">
        ...
        <div className="grid-2">
          ...
          <FormRow
            ...
            required
            hint={
              allowedTagStates.length === 0
                ? "No onward state is permitted from here."
                : `Permitted from ${humanise(spec?.state ?? "not_started")}: ${allowedTagStates.map(humanise).join(", ")}`
            }
          >
            <SelectInput
              id="tg-state"
              value={tagState}
              onChange={(v) => setTagState(v)}
              options={[
                {
                  value: spec?.state ?? "not_started",
                  label: `${humanise(spec?.state ?? "not_started")} (current)`,
                },
                ...allowedTagStates.map((v) => ({ value: v, label: humanise(v) })),
              ]}
        ...
        <div className="grid-2">
          <FormRow label="Agreed by" htmlFor="tg-by" error={tagErrors["tg-by"]}>
            <TextInput id="tg-by" value={agreedBy} onChange={setAgreedBy} error={tagErrors["tg-by"]} />
          </FormRow>
        ...
        <FormActions>
          <button type="button" className="btn btn--primary" onClick={saveTags} disabled={savingTags}>
            {savingTags ? "Saving…" : "Save tags specification"}
          </button>
        </FormActions>
        <RequiredLegend />
        <Banner tone="info" title="Reprocessing re-opens this">
          ...
          specification has been set (v2.0 §6.5 exception). That is why <em>Agreed</em> is not terminal and a
          re-issued specification is a state of its own. The batch code itself is recorded on the stock and
          deliberately excluded from stock allocation — see{" "}
