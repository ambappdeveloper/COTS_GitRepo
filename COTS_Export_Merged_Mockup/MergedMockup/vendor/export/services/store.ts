...
import { COUNTRY_PROFILES, countryName, reviewPriceLegsFor } from "../domain/variants";
import {
...
import type {
  ...
  ReviewFeedbackPrice,
  TagSpecification,
  TagSpecificationAction,
  TagSpecificationEvent,
...
/* ================================================================== *
 * THE OPERATING COUNTRY, AS A SCOPE ON EVERY LIST
 *
 * WHY THIS EXISTS. Instruction of 9 September 2026:
 *
 *   "Export should connect to the country in the header. If country is Tanzania,
 *    the user should view only Tanzania contracts; when they switch country the
 *    contract list should update. This should apply to any screen affected by
 *    country."
 *
 * Before this, the header's country drove Core and the Shared modules and nothing
 * else: a reviewer could switch to Tanzania and still be reading a Sudan contract,
 * with the header saying one country and the record saying another. That is the same
 * defect `CountrySync` was written to remove one module along — two answers to one
 * question, and no way to tell which one a screen is obeying.
 *
 * WHY IT IS HERE AND NOT ON THE SCREENS. Every list in this module goes through an
 * `api.list*` accessor below — twenty-nine of them, read by some sixty call sites
 * across the pages, the home counters, the dashboard and the merged application's
 * Actions Inbox. A filter written on a screen is a filter the next screen forgets, and
 * the failure is silent: a list quietly shows another country's records and nothing
 * says so. Filtering in the accessor means a screen cannot opt out by accident, and a
 * list added later is scoped before anyone thinks about it.
 *
 * WHY THE LISTS REFRESH BY THEMSELVES. `setCountryScope` calls `notify()`, and
 * `useAsync` in `pages/hooks.ts` already re-fetches on every store notification. So
 * switching the country in the header re-runs every list on screen with no page code
 * and no reload. That is the whole of "the contract list should update".
 *
 * WHAT THE SCOPE IS NOT. It is not a permission. Which countries a person may work in
 * is decided by the Core account's `countryScope`, which is what the header's selector
 * already offers and what makes it read-only for a single-country user. This is the
 * one country of those they are working in now. Nothing here grants or refuses access
 * — see the note on `getContract` below about records reached by a direct link.
 * ================================================================== */

/**
 * How each record type answers "which country is this?".
 *
 * Three shapes, and the third is the one to be careful with.
 *
 *  1. **The record says so.** `Contract.origin`, `Shipment.country`,
 *     `Opportunity.origin`, `StockLot.origin`, `WarehouseRequest.country`,
 *     `AdvancePayment.country`, `MovementLeg.originCountry`,
 *     `ReceivingLocationPlan.country`.
 *  2. **A parent says so.** Cargo readiness, stuffing requests, transport requests,
 *     claims, feedback, insurance and risks read their shipment; export contracts read
 *     their purchase contract; pre-clearance packs read their export contract's.
 *  3. **Nothing says so.** Vessel calls and freight rates are reference data shared
 *     across countries. The sourcing and procurement records — seasonal plans,
 *     budgets, purchase agreements and orders, intake receipts, funds, agent balances
 *     and the production plan — carry no country at all, on the record or on any
 *     parent. Those lists are left unscoped and the reason is stated on the accessor,
 *     because a country filter applied to a record that has no country would empty
 *     the screen without being able to say why. **[OPEN] Whether those records should
 *     carry a country** is a modelling question for the sourcing design, not something
 *     to be settled by a filter.
 *
 * `Shipment.country` is used in preference to its contract's origin even though the two
 * are the same on every seeded row and a test holds them so. The country is on the
 * record; reading it from a parent when the record states it would be inventing a
 * derivation, and it would put every shipment behind a contract lookup.
 */
let countryScope: CountryUnit | null = null;

/**
 * Scope every list to one country, or to none.
 *
 * `null` means unscoped, which is what the module does when the session does not name a
 * country. Deliberately not "default to Sudan": a defaulted scope silently hides four
 * countries' records and looks identical to there being none, which is the failure mode
 * `activeCountryOf` already refuses for the same reason.
 */
export function setCountryScope(next: CountryUnit | null): void {
  if (countryScope === next) return;
  countryScope = next;
  notify();
}

/** The country every list is currently filtered to, or `null` where none is set. */
export function activeCountryScope(): CountryUnit | null {
  return countryScope;
}

/**
 * Keep the rows belonging to the active country.
 *
 * A row whose country cannot be derived is **kept**, not dropped. An underivable
 * country means a broken link — a shipment naming a contract that is not there — and
 * that is a defect in the data, not a country. Dropping such a row would hide the
 * defect behind a filter, and a reviewer would read the shorter list as correct. The
 * seeded data has no such row and a test in `routes.test.tsx` holds it that way, so in
 * practice this branch never fires; it exists so that when it does, the row is visible.
 */
function scoped<T>(rows: T[], countryOf: (row: T) => CountryUnit | undefined): T[] {
  if (!countryScope) return rows;
  return rows.filter((row) => {
    const c = countryOf(row);
    return c === undefined || c === countryScope;
  });
}
...
function shipmentCountry(shipmentId: string | undefined): CountryUnit | undefined {
  if (!shipmentId) return undefined;
  return store.shipments.find((s) => s.id === shipmentId)?.country;
}

function exportContractCountry(exportContractId: string | undefined): CountryUnit | undefined {
  if (!exportContractId) return undefined;
  const e = store.exportContracts.find((x) => x.id === exportContractId);
  return e ? contractCountry(e.contractId) : undefined;
}
...
/* ------------------------------------------------------------------ *
 * Shipment bank details — one resolver, two callers
 * ------------------------------------------------------------------ */

/**
 * Turns a bank id and a branch name into the record a shipment holds, or says why it cannot —
 * 11 September 2026.
 *
 * WHY IT IS A FUNCTION AND NOT TWO COPIES OF THE SAME CHECKS. The bank can now arrive by two
 * routes: with the split, from the New shipment form, and after it, from the Summary card that
 * Finance answers. Two entry points validating a field their own way is how two screens come to
 * disagree about what is allowed — the defect `reviewPriceLegsFor` was written to prevent on the
 * review dialog, one screen over. So both go through here, and the create form cannot file a
 * value the later edit would refuse.
 *
 * THE COUNTRY DECIDES WHETHER IT MAY BE HELD AT ALL. `requiresShipmentBankDetails` is true for
 * Sudan and false for the other four (9 September 2026). A bank sent for a country that does not
 * request one is refused rather than quietly dropped: silently discarding a value somebody typed
 * is worse than telling them it does not apply here.
 *
 * NOTHING IS MANDATORY. No bank at all is a valid answer — it returns `undefined`, which is what
 * leaves the `shipment_bank_details` milestone unanswered and the Finance row in the Actions
 * Inbox. No source makes the bank a precondition for anything, and decision D-10 is the standing
 * rule against inventing a gate.
 */
function resolveShipmentBankDetails(
  country: Shipment["country"],
  draft: { bankId?: string; bankBranch?: string; providedBy?: string },
):
  | { ok: true; value: ShipmentBankDetails | undefined }
  | { ok: false; reason: string } {
  const profile = COUNTRY_PROFILES[country];

  if (!draft.bankId) {
    /* A branch with no bank is not a partial answer, it is an inconsistent one: the branch list
       is keyed by the bank, so a branch alone names nothing. */
    if (draft.bankBranch) {
      return { ok: false, reason: "Select the bank before its branch." };
    }
    return { ok: true, value: undefined };
  }

  if (profile && !profile.requiresShipmentBankDetails) {
    return {
      ok: false,
      reason:
        `${profile.name} does not require bank details on a shipment, so none is recorded there. ` +
        "The requirement is configured per country.",
    };
  }

  const bank = COUNTERPARTIES.find((c) => c.id === draft.bankId && c.type === "bank");
  if (!bank) return { ok: false, reason: "Select the bank from the counterparty master." };

  const branches = bankBranchesFor(bank.id);
  if (draft.bankBranch && !branches.includes(draft.bankBranch)) {
    return {
      ok: false,
      reason: `${bank.name} has no branch called “${draft.bankBranch}” in master data.`,
    };
  }
  ...
  return {
    ...
    value: {
      bankId: bank.id,
      /* The name as it reads now, kept on the record so that renaming the master entry does not
         rewrite what a shipped record says it went through. */
      bankName: bank.name,
      bankBranch: draft.bankBranch || undefined,
      providedOn: TODAY,
      providedBy: draft.providedBy?.trim() || undefined,
    },
  };
}
...
export const api = {
  /*
   * Every list below is scoped to the active country — see THE OPERATING COUNTRY above.
   * The `get*` readers are deliberately **not** scoped: a record reached by a direct
   * link opens, and the screen says which country it belongs to. A link that returns
   * "not found" because the reader happened to be in another country is a broken link,
   * and it teaches people that COTS links cannot be shared.
   */

  /* --- contracts --- */
  listContracts: () => delay(clone(scoped(store.contracts, (c) => c.origin))),
  getContract: (id: string) => delay(clone(store.contracts.find((c) => c.id === id)) ?? undefined),

  /* --- cargo readiness — the country comes from its shipment --- */
  listCargoReadiness: () =>
    delay(clone(scoped(store.cargoReadiness, (r) => shipmentCountry(r.shipmentId)))),

  /* --- export contracts — the country comes from the purchase contract --- */
  listExportContracts: () =>
    delay(clone(scoped(store.exportContracts, (e) => contractCountry(e.contractId)))),
  getExportContract: (id: string) =>
    delay(clone(store.exportContracts.find((e) => e.id === id)) ?? undefined),
  listPreclearancePacks: () =>
    delay(clone(scoped(store.preclearancePacks, (p) => exportContractCountry(p.exportContractId)))),

  /* --- shipments --- */
  listShipments: () => delay(clone(scoped(store.shipments, (s) => s.country))),
  getShipment: (id: string) => delay(clone(store.shipments.find((s) => s.id === id)) ?? undefined),

  /*
   * --- vessels ---
   * Not scoped. A vessel call is a ship at a port and belongs to no operating unit; the
   * same call carries cargo for more than one. Filtering it by country would hide the
   * ship from the country whose cargo is on it.
   */
  ...
  /*
   * --- material & transport ---
   * `MaterialPurchase.origin` and `TransportRequest.origin` are **places** — "Gedaref",
   * "Kosti" — not countries, so neither is a country field and neither is read as one.
   * A transport request that names a shipment is scoped through it; material purchases
   * and their receipts carry no country on the record or on any parent and are left
   * unscoped. [OPEN] whether they should carry one.
   */
  ...
  listTransportRequests: () =>
    delay(clone(scoped(store.transportRequests, (t) => shipmentCountry(t.shipmentId)))),
  ...
  listOpportunities: () => delay(clone(scoped(store.opportunities, (o) => o.origin))),
  ...
  listStockLots: () => delay(clone(scoped(store.stockLots, (l) => l.origin))),
  /* The production plan is a facility's week; it names no country. [OPEN] as above. */
  listProductionPlan: () => delay(clone(store.productionPlan)),
  listWarehouseRequests: () => delay(clone(scoped(store.warehouseRequests, (w) => w.country))),

  /* --- workflow v2.0: country prerequisites (phase 07) --- */
  listAdvancePayments: () => delay(clone(scoped(store.advancePayments, (a) => a.country))),
   ...
   * Not scoped. A rate is quoted per loading port and destination, and the port implies
   * the country rather than the other way about; a rate table filtered by country would
   * hide the rate for a transit port that sits in the neighbouring one — Djibouti for
   * Ethiopia, Douala for Chad, which are exactly the routes this module has.
   */
  listFreightRates: () => delay(clone(store.freightRates)),

  /* --- workflow v2.0: movement & stuffing request (phase 10) --- */
  listMovementLegs: () => delay(clone(scoped(store.movementLegs, (m) => m.originCountry))),
  getMovementLeg: (id: string) => delay(clone(store.movementLegs.find((m) => m.id === id)) ?? undefined),
  listStuffingRequests: () =>
    delay(clone(scoped(store.stuffingRequests, (r) => shipmentCountry(r.shipmentId)))),

  /* --- workflow v2.0: close-out (phase 16) --- */
  listCustomerFeedback: () =>
    delay(clone(scoped(store.customerFeedback, (f) => contractCountry(f.contractId)))),
  listBuyerClaims: () => delay(clone(scoped(store.buyerClaims, (c) => contractCountry(c.contractId)))),
  listInsurancePolicies: () =>
    delay(clone(scoped(store.insurancePolicies, (p) => shipmentCountry(p.shipmentId)))),
  listInsuranceIncidents: () =>
    delay(clone(scoped(store.insuranceIncidents, (i) => shipmentCountry(i.shipmentId)))),

  /*
   * --- sourcing and procurement ---
   * None of the records below carries a country, on itself or on any parent: a seasonal
   * purchase plan, a budget, a purchase agreement, a purchase order, an intake receipt,
   * a fund and an agent balance are all identified by season, agent, commodity and
   * reference. They are left unscoped rather than filtered to nothing.
   *
   * [OPEN] Sourcing records carry no operating country. Every one of them is in practice
   * a single country's — the seeded rows are all Sudan's — and once a second country is
   * loaded, these screens will show both with no way to tell them apart. Adding a country
   * to those records is a modelling decision for the sourcing design; it is recorded here
   * rather than guessed at, because the only alternative available to a filter is to hide
   * every row.
   *
  ...
  /* --- risks --- */
  /*
   * A risk raised on a shipment is that shipment's country; a standalone risk names no
   * shipment and no country, so it is carried through unscoped and stays visible.
   */
  listRisks: () =>
    delay(
      clone([
        ...scoped(store.shipments, (s) => s.country).flatMap((s) => s.risks),
        ...store.standaloneRisks,
      ]),
    ),
  ...
  async createContract(
    ...
    /*
     * A contract is raised in the country the session is working in — 9 September 2026.
     *
     * The form fixes Origin to the active country rather than offering the list, so this
     * cannot normally be reached. It is here for the same reason every other rule is
     * checked twice: a rule that only lives on a screen is a rule the next caller skips,
     * and a contract raised in the wrong country would then be invisible to the country
     * that raised it — the list it would appear in is scoped to a country the creator is
     * not in. Unscoped (no country in the session), any origin is accepted.
     *
     * Checked before the field validation, and deliberately: the origin is not a field
     * this screen asks for any more, so answering a wrong country with "Buyer is required"
     * would name the wrong problem and point at a field that is not the one at fault.
     */
    if (countryScope && draft.origin && draft.origin !== countryScope) {
      return delay(
        failResult(
          `The contract's origin is ${countryName(draft.origin as CountryUnit)}, but this session is working in ${countryName(countryScope)}. Switch country in the header to raise a contract there.`,
        ),
      );
    }
    ...
    const created: Contract = {
      ...
      /* P3 exists as a phase with no legacy screen: the reviewing functions start out pending.
         Processing was one of them until the instruction of 8 September 2026 took it off the
  ...
  async createShipment(draft: {
    ...
    /**
     * The bank and branch, where the raiser already knows them — 11 September 2026.
     *
     * Optional, and optional is the whole design. Leaving them out is the 9 September
     * behaviour unchanged: the split is raised without them and the `shipment_bank_details`
     * milestone leaves a row for Finance. Supplying them records the details and completes
     * that milestone in the same operation, so the task is never raised for an answer that
     * arrived with the record.
     */
    bankId?: string;
    bankBranch?: string;
    /**
    ...
    /* The bank is resolved before anything is written, through the same function the Summary
       card's save goes through — so the create form cannot file a bank the later edit would
       refuse. The shipment's country is the contract's origin, which is what this record is
       about to be given, not the session's. */
    const bank = resolveShipmentBankDetails(contract.origin, {
      bankId: draft.bankId,
    ...
    const created: Shipment = {
      ...
      milestones: [
        ...
        /* Completed here only when the bank arrived with the record. Left off otherwise, which
           is how the Actions Inbox raises the row for Finance — an absent milestone is an
           unanswered one. */
        ...(bank.value
          ? [
              {
                key: "shipment_bank_details" as const,
                state: "completed" as const,
                actualDate: TODAY,
                ownerName: bank.value.providedBy,
              },
            ]
          : []),
      ],
      ...
      audit: [
        ...
        /* A second line rather than a longer first one: the bank is a separate fact, recorded
           by the raiser rather than by Finance, and the trail should say which function
           answered it. */
        ...(bank.value
          ? [
              {
                id: `au-${++auditSeq}`,
                at: `${TODAY}T12:00:00Z`,
                actor: bank.value.providedBy ?? "Dubai Execution",
                actorRole: "dubai_execution" as const,
                action: "Updated" as const,
                entity: `Shipment ${contract.contractNo}.${seq}`,
                note: `Bank details recorded when the split was raised: ${bank.value.bankName}${
                  bank.value.bankBranch ? ` · ${bank.value.bankBranch}` : ""
                }.`,
              },
            ]
          : []),
      ],
      /* Was the execution plan's assigned owner. The contract's own Dubai Execution owner is
         what remains once planning is gone, and it is the same person in every captured
         record — the plan inherited it from the contract in the first place. */
      assignedTo: contract.assignedDubaiExecution || template.assignedTo,
      blocked: undefined,
    };
   ...
   * finance once split so they enter bank details (bank name, branch name)."
   *
   * THE NOTIFICATION IS THE MILESTONE. Raising a split in a country whose profile requires bank
   * details leaves `shipment_bank_details` unanswered, and an unanswered milestone owned by
   * Finance is a row in the one Actions Inbox — see `src/integration/exportTasks.ts`. This
   * operation is what clears it: it writes the details and completes the milestone in the same
   * step, so the task cannot survive the answer.
   *
   * THE BANK IS READ FROM THE MASTER, THE NAME IS KEPT ON THE RECORD. `bankId` is the governed
   * counterparty; `bankName` is what that record was called when Finance recorded it. Both are
   * stored, so a bank renamed in master data does not silently rewrite what a shipped record
   * says it went through.
   *
   * NOT A GATE. A country that requires the details and does not have them is not blocked from
   * anything — no source makes the bank a precondition, and decision D-10 is the standing rule
   * against inventing one.
   */
  async setShipmentBankDetails(
    id: string,
    draft: { bankId: string; bankBranch?: string; providedBy?: string },
  ): Promise<Result<Shipment>> {
    const s = store.shipments.find((x) => x.id === id);
    ...
    /* Shared with `createShipment` since 11 September 2026 — see `resolveShipmentBankDetails`.
       This caller additionally requires a bank: the card's own action is "record the bank", so
       an empty save here is a mistake rather than the deliberate "leave it for Finance" that an
       empty create form means. */
    const resolved = resolveShipmentBankDetails(s.country, draft);
    if (!resolved.ok) return delay(failResult(resolved.reason));
    if (!resolved.value) return delay(failResult("Select the bank from the counterparty master."));

    s.bankDetails = resolved.value;
    s.milestones = [
      ...s.milestones.filter((m) => m.key !== "shipment_bank_details"),
      {
        key: "shipment_bank_details",
        state: "completed",
        actualDate: TODAY,
        ownerName: draft.providedBy?.trim() || undefined,
      },
    ];
    pushAudit(id, {
      actor: draft.providedBy?.trim() || "Finance",
      actorRole: "finance",
      action: "Updated",
      entity: `Shipment ${s.shipmentNo}`,
  ...
  async requestExportContract(draft: {
    ...
    const contract = store.contracts.find((c) => c.id === draft.contractId);
    if (!contract) return delay(failResult("Select the purchase contract this request is raised against."));
    ...
    /*
     * `<contract no>-R<n>` — the contract's own sequence, since 8 September 2026.
     *
     * It was `<planning no>-R<n>` until execution planning was removed, which is the format
     * every *captured* request still uses (PC-2041.1-R1). Those are left as captured: a
     * request number is on a record the business issued, and renumbering history to match a
     * new rule would be inventing a past. So the sequence is computed by counting the
     * requests already on the contract rather than by parsing their numbers, which is what
     * makes the two formats able to coexist. A second request against one contract is
     * allowed: no source forbids it, and a rejected request being re-raised is the obvious
     * case.
     */
    const onContract = store.exportContracts.filter((e) => e.contractId === contract.id);
    const next = onContract.length + 1;

    const created: ExportContract = {
      id: `ec-new-${contract.id}-${next}`,
      requestNo: `${contract.contractNo}-R${next}`,
      ...
      exportForms: [],
      consumption: [],
      /* Read from the contract: large volume describes one export contract consumed across
         several shipments, which is exactly this record. */
  ...
  /**
   * Records what the Ministry of Trade returns — 11 September 2026.
   *
   * THE GAP THIS CLOSES. It is gap 1 of the five in the process-review meeting pack of
   * 7 September: *"The request can now be raised, but nothing records what the ministry
   * returns — contract number, dates, actual quantity — or the EX forms issued against it.
   * Consumption can be recorded; issuance cannot. The middle of the pre-clearance chain is
   * missing."* Every field on the Issuance card has rendered since v1.0 and none could be
   * written: the record could only ever show the seeded issuance, never one the mock-up made.
   *
   * THE REQUEST IS NOT TOUCHED. `requestNo`, `contractId`, `requestedOn`,
   * `requestedQuantityMt`, `exportingEntity` and `unitPrice` are not in the draft and are not
   * written. A request is the record of what the business *sent*; correcting it after the fact
   * would rewrite the thing the ministry answered. Confirmed in review, 11 September.
   *
   * THE STATUS FOLLOWS THE ACTION. §6.14 states the transition Requested → Under process →
   * Issued. Recording the contract number and the issuance date *is* the issuance, so `issued`
   * is derived here rather than chosen on a form — the rule the tag specification moved to on
   * 9 September, where a selected state let a record say `agreed` with nobody having agreed.
   *
   * THE EXPIRY IS REQUIRED, and that is the one judgement in this operation. §6.14: *"Export
   * contract: Requested → Under process → Issued, with an expiry. [AS-IS]"* — the source states
   * that an issued contract has one, and rule R13 (the validity risk on the detail screen and
   * the shipment's own expiry check) has nothing to work with when it is absent. Required on
   * the transition to Issued, not on the record.
   */
  async issueExportContract(
    id: string,
    draft: {
    ...
    const e = store.exportContracts.find((x) => x.id === id);
    if (!e) return delay(failResult("Export contract not found."));

    const contract = store.contracts.find((c) => c.id === e.contractId);
    ...
    if (profile && !profile.usesExportContract) {
      return delay(
        failResult(
          `${profile.name} does not use an export contract${
            profile.startsFromCommercialInvoice ? " — its customs process starts from a commercial invoice" : ""
          }, so no issuance is recorded there.`,
        ),
      );
    }

    const no = draft.exportContractNo.trim();
    if (!no) return delay(failResult("Enter the export contract number the ministry issued."));
    /* One number, one contract. The legacy estate has no such constraint and the balance
       review depends on the number identifying the document. */
    const clash = store.exportContracts.find(
      (x) => x.id !== id && x.exportContractNo && x.exportContractNo.trim() === no,
    );
    if (clash)
      return delay(failResult(`Export contract ${no} is already recorded against ${clash.requestNo}.`));

    if (!draft.issuanceDate) return delay(failResult("Enter the issuance date."));
    if (!draft.expiryDate)
      return delay(
        failResult(
          "Enter the expiry date. §6.14 states an issued export contract carries one, and the validity check (rule R13) has nothing to measure without it.",
        ),
      );
    if (draft.expiryDate <= draft.issuanceDate)
      return delay(failResult("The expiry date must be after the issuance date."));

    if (draft.actualQuantityMt !== undefined && !(draft.actualQuantityMt > 0))
      return delay(failResult("The actual quantity must be greater than zero, or left blank."));

    /* The bank is a governed counterparty, as it is on the shipment since 11 September. The
       branch is keyed by the bank, so a branch alone names nothing. */
    let bankName: string | undefined;
    if (draft.actualBankId) {
      const bank = COUNTERPARTIES.find((c) => c.id === draft.actualBankId && c.type === "bank");
      if (!bank) return delay(failResult("Select the bank from the counterparty master."));
      bankName = bank.name;
      const branches = bankBranchesFor(bank.id);
      if (draft.actualBankBranch && !branches.includes(draft.actualBankBranch)) {
        return delay(failResult(`${bank.name} has no branch called “${draft.actualBankBranch}” in master data.`));
      }
    } else if (draft.actualBankBranch) {
      return delay(failResult("Select the bank before its branch."));
    }

    e.exportContractNo = no;
    e.issuanceDate = draft.issuanceDate;
    e.expiryDate = draft.expiryDate;
    e.actualExporterName = draft.actualExporterName?.trim() || undefined;
    e.actualBank = bankName;
    e.actualBankBranch = draft.actualBankBranch || undefined;
    e.actualQuantityMt = draft.actualQuantityMt;
    e.scannedContractName = draft.scannedContractName?.trim() || undefined;
    if (draft.notes !== undefined) e.notes = draft.notes.trim() || undefined;
    /* Derived, never chosen — see the note above. */
    e.status = "issued";

    notify();
    return delay(okResult(clone(e)));
  },

  /**
   * Issues one EX form against an export contract — 11 September 2026.
   *
   * The second half of meeting-pack gap 1: *"EX form ISSUE has no screen. Consumption can be
   * recorded; issuing a form cannot."* `consumeExportForm` has existed since v1.0 and marks a
   * form Used; nothing created one, so only the seeded forms could ever be consumed.
   *
   * SUDAN, AS CONFIGURATION. `usesExportForms` is true for Sudan and false for the other four —
   * both sources agree, and the workshop notes say it in these words: *"Ex form is not
   * applicable in all counties. Mainly in Sudan."* The refusal reads from the profile rather
   * than from a country code.
   *
   * ISSUED, NOT UNDER PROCESSING. `ExportFormStatus` is Under processing → Issued → Used, and
   * this operation is the *issue*: the bank has produced the form. A form created as
   * `under_processing` would be a form nobody had issued.
   *
   * THE R5 TOTAL IS NOT A GATE. `validateExportFormTotal` already reports when the forms do not
   * sum to the contract quantity, and the detail screen shows it as a risk banner. It stays a
   * report: no source makes it blocking, the legacy estate saved three different totals for one
   * shipment, and decision D-10 is the standing rule against inventing a gate. The screen warns
   * before saving and the banner persists after.
   */
  async addExportForm(
    id: string,
    draft: { formNo: string; quantityMt: number; issuedOn?: string; psFileNo?: string },
  ): Promise<Result<ExportContract>> {
    const e = store.exportContracts.find((x) => x.id === id);
    if (!e) return delay(failResult("Export contract not found."));
    ...
    const contract = store.contracts.find((c) => c.id === e.contractId);
    const profile = contract ? COUNTRY_PROFILES[contract.origin] : undefined;
    if (profile && !profile.usesExportForms) {
      return delay(
        failResult(
          `EX forms do not apply in ${profile.name} — the workshop notes record them as mainly Sudan. None is issued there.`,
        ),
      );
    }

    const formNo = draft.formNo.trim();
    if (!formNo) return delay(failResult("Enter the EX form number."));
    if (e.exportForms.some((f) => f.formNo.trim() === formNo))
      return delay(failResult(`EX form ${formNo} is already issued against this export contract.`));
    if (!(draft.quantityMt > 0)) return delay(failResult("The form quantity must be greater than zero."));
    ...
    e.exportForms = [
      ...e.exportForms,
      {
        ...
        quantityMt: draft.quantityMt,
        status: "issued",
        issuedOn: draft.issuedOn || TODAY,
        psFileNo: draft.psFileNo?.trim() || undefined,
      },
    ];
    notify();
    return delay(okResult(clone(e)));
  ...
  /**
   * Removes an EX form that was issued in error — 11 September 2026.
   *
   * A form that has been *used* is refused: consumption is recorded against its number, on the
   * contract's own `consumption` list and on the clearance it was declared under, and removing
   * the form would leave both pointing at nothing. That is rule R7's own reasoning — a used form
   * is spent — applied to deletion rather than to re-use.
   */
  async removeExportForm(id: string, formNo: string): Promise<Result<ExportContract>> {
    const e = store.exportContracts.find((x) => x.id === id);
    if (!e) return delay(failResult("Export contract not found."));
    const form = e.exportForms.find((f) => f.formNo === formNo);
    if (!form) return delay(failResult("That EX form is not on this export contract."));
    if (form.status === "used")
      return delay(
        failResult(
          `EX form ${formNo} has been used and cannot be removed — consumption is recorded against its number.`,
        ),
      );
    e.exportForms = e.exportForms.filter((f) => f.formNo !== formNo);
    notify();
    return delay(okResult(clone(e)));
  },
  ...
  async recordReviewFeedback(
    ...
    opts: { respondedBy?: string; comment?: string; prices?: ReviewFeedbackPrice[] } = {},
  ): Promise<Result<Contract>> {
    ...
    if (!row) {
      return delay(
        failResult(
          `${role} is not one of the functions the contract review notifies.`,
        ),
      );
    }
    ...
    /*
     * Prices per country of the route — Finance only, 9 September 2026.
     *
     ...
     *
     * The countries are not the caller's to choose: they are the contract's origin and, where
     * the profile names one, its transit country. A price against any other country is refused
     * rather than stored, because a figure filed under a country the cargo never enters is worse
     * than no figure — and refusing it here means a second screen cannot quietly do it either.
     */
    if (opts.prices?.length) {
      if (role !== "finance") {
        return delay(failResult("Only Finance records a price on the contract review."));
      }
      const allowed = reviewPriceLegsFor(c);
      for (const p of opts.prices) {
        const leg = allowed.find((l) => l.leg === p.leg);
        if (!leg) {
          return delay(
            failResult(
              `${COUNTRY_PROFILES[c.origin]?.name ?? c.origin} has no ${p.leg} leg to price — the route settles which countries are priced, not the form.`,
            ),
          );
        }
        if (p.countryName !== leg.countryName) {
          return delay(failResult(`The ${p.leg} leg is ${leg.countryName}, not ${p.countryName}.`));
        }
        if (!(p.amount.amount > 0)) {
          return delay(failResult(`Enter a price for ${leg.countryName} greater than zero, or leave it blank.`));
        }
      }
    }
    ...
    if (role === "finance") row.prices = opts.prices?.length ? clone(opts.prices) : undefined;
    notify();
    return delay(okResult(clone(c)));
  },
  ...
  /**
   * One leg of the tag specification exchange — 9 September 2026.
   *
   * "Origin execution will send tag and Dubai execution will confirm or add amendment; there is
   * a workflow."
   *
   * `setTagSpecification` used to take a whole specification and a target state, which suited a
   * single form filled in by one person and suits a two-party hand-off badly: it let any caller
   * assert any state without saying who had acted or why. This takes the *action* instead, works
   * out the state from it, and appends the leg to the exchange — so the record cannot hold a
   * state that nobody performed.
   *
   * WHO MAY DO WHAT IS SHOWN, NOT ENFORCED. Nothing here checks that the caller belongs to the
   * party that owns the action. That is the same position the cross-functional review takes and
   * for the same reason: which role may act is a C01 permission question, and inventing the rule
   * one screen at a time is how two screens end up disagreeing about it. The party is recorded
   * on the leg, so the exchange still says who did what.
   *
   * WHAT EACH ACTION NEEDS:
   *   · `sent`      — the specification content, and the buyer-option rule §6.5 activity 3 states.
   *   · `amended`   — a note. An amendment that does not say what to change is not an amendment.
   *   · `confirmed` — nothing; a note is allowed.
   *   · `reissued`  — nothing. Reprocessing is the event; the new tag follows as a fresh `sent`.
   */
  async recordTagSpecificationAction(
    contractId: string,
    action: TagSpecificationAction,
    payload: {
      by?: string;
      note?: string;
      /* Only read on `sent`, which is the only leg that carries the specification itself. */
      option?: TagSpecification["option"];
      customTags?: string[];
    ...
    const current: TagSpecificationState = c.tagSpecification?.state ?? "not_started";
    const target: TagSpecificationState = action;
    const check = canTransition(TAG_SPECIFICATION_TRANSITIONS, current, target);
    if (!check.allowed) return delay(failResult(check.reason));

    if (action === "amended" && !payload.note?.trim()) {
      return delay(failResult("An amendment must say what is to change."));
    }

    const existing = c.tagSpecification;
    const option = action === "sent" ? (payload.option ?? existing?.option ?? "standard") : (existing?.option ?? "standard");
    const customTags =
      action === "sent" ? (payload.customTags ?? existing?.customTags ?? []) : (existing?.customTags ?? []);

    if (action === "sent" && option === "buyer" && customTags.length === 0) {
      return delay(
        failResult(
    ...
    /* The party that owns the action. Origin Execution sends; Dubai Execution answers. */
    const party: Role = action === "sent" ? "partner_execution" : "dubai_execution";
    const event: TagSpecificationEvent = {
      id: `tge-${contractId}-${(existing?.exchange.length ?? 0) + 1}`,
      action,
      party,
      by: payload.by?.trim() || undefined,
      at: TODAY,
      note: payload.note?.trim() || undefined,
      attachmentFileName:
        action === "sent" ? payload.attachmentFileName?.trim() || undefined : undefined,
    };
    ...
    c.tagSpecification = {
      ...
      customTags,
      batchCodeOnTag:
        action === "sent" ? (payload.batchCodeOnTag ?? existing?.batchCodeOnTag ?? false) : (existing?.batchCodeOnTag ?? false),
      /* Confirmation is what "agreed" means now, so the agreed fields are written by that leg
         and by no other. */
      agreedOn: action === "confirmed" ? TODAY : existing?.agreedOn,
      agreedBy: action === "confirmed" ? (payload.by?.trim() || existing?.agreedBy) : existing?.agreedBy,
      reissuedAfterReprocessingOn:
        action === "reissued" ? TODAY : existing?.reissuedAfterReprocessingOn,
      /* The attachment shown on the record is the one the latest send carried. Earlier ones are
         not lost — each is on its own leg of the exchange, which is what answers the question
         left open when the attachment was added on 8 September. */
      attachmentFileName:
        action === "sent"
          ? payload.attachmentFileName?.trim() || existing?.attachmentFileName
          : existing?.attachmentFileName,
      exchange: [...(existing?.exchange ?? []), event],
      note: existing?.note,
    };
    notify();
    return delay(okResult(clone(c)));
