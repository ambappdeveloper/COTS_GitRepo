 ...
 *   · the execution plan carried the instrument alone — every captured plan held `DA`,
 ...
 * carrying a tenor. The contract's drop-down and the deal's read the second.
 *
 * WHO READS THE FIRST — nobody, since 8 September 2026. Execution planning was removed on the
 * grounds that split shipments replace it, and the plan's `exportContractPaymentTerms` was the
 * only field in the model that carried an instrument on its own. The list is kept rather than
 * deleted for two reasons: it is the parent of the term list, and every term's `instrument`
 * joins to it; and it is a governed C03 domain in its own right, which a screen removal does
 * not undo. If the split shipment turns out to carry the export contract's terms, this is the
 * list it should read — that is the open question, and it is why the list was not thrown away
 * along with its only reader.
 *
