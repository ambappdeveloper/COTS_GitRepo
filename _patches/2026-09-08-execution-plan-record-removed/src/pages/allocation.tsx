              ...
              <tbody>
                {readiness.map((r) => (
                  <ReadinessRow
                    ...
                    record={r}
                    actor={actor}
                    shipment={shipments.find((s) => s.id === r.shipmentId)}
                  />
                ))}
              </tbody>
            </table>
...
function ReadinessRow({
  ...
  return (
    <tr>
      <td>
        {/* The planning number was the first line here until execution planning was removed
            on 8 September 2026. Readiness hangs off the shipment now, so the shipment is the
            identity of the row rather than a sub-line under the plan's. */}
        {shipment ? (
      ...
      <td>
        ...
        <Dialog
          open={statusOpen}
          title={`Change cargo status${shipment ? ` on ${shipment.shipmentNo}` : ""}`}
