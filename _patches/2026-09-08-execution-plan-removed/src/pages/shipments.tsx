...
export function ShipmentDetail() {
  ...
  const plan = (plans.data ?? []).find((p) => p.id === s.executionPlanId);
  const readiness = (cargo.data ?? []).find((cr) => cr.executionPlanId === s.executionPlanId);
  ...
  return (
    <>
      ...
      <div className="page">
              ...
              <SummaryCard title="Terms">
                <FieldGrid
                  ...
                  fields={[
                    ...
                    { label: "Execution plan", value: plan?.planningNo, behaviour: "inherited" },
        ...
        {activeTab === "planning" ? (
          <>
            <CollapsibleSection title="Execution planning">
              {plan ? (
                <FieldGrid
                  fields={[
                    { label: "Planning number", value: plan.planningNo, behaviour: "readonly" },
                    { label: "Status", value: humanise(plan.status) },
                    {
                      label: "Planned quantity",
                      value: formatMt(plan.plannedQuantityMt),
                      behaviour: "required",
                    },
                    { label: "Port of loading", value: portName(plan.portOfLoadingId) },
                    { label: "Shipper", value: plan.shipperName },
                    { label: "Shipper on behalf of", value: plan.shipperOnBehalfOf },
                    { label: "Bank", value: plan.bank },
                    { label: "Seasonality", value: plan.seasonality },
                    { label: "Cargo source", value: plan.cargoSource, behaviour: "required" },
                    { label: "Urgency", value: humanise(plan.urgency) },
                    { label: "Assigned to", value: plan.assignedTo },
                    { label: "To be shipped before", value: formatDate(plan.toBeShippedBefore) },
                  ]}
                />
              ) : (
                <EmptyState title="No execution plan linked" />
              )}
            </CollapsibleSection>
                  ...
                  <FieldGrid
                    fields={[
                      ...
                      { label: "Quantity ready", value: formatMt(readiness.qtyReadyMt) },
                      {
                        ...
                        value: formatMt(Math.max(0, (plan?.plannedQuantityMt ?? 0) - readiness.qtyReadyMt)),
                        behaviour: "calculated",
                      },
...
export function ShipmentForm({ mode }: { mode: "create" | "edit" }) {
  ...
  if (mode === "edit" && existing && !hydrated) {
    ...
    setPlanId(existing.executionPlanId);
    ...
    setReadiness(existing.commodityReadinessDate ?? "");
    setLastShip(existing.lastShippingDate ?? "");
    setHydrated(true);
  }

  const contract = (contracts.data ?? []).find((c) => c.id === contractId);
  const contractPlans = useMemo(
    () => (plans.data ?? []).filter((p) => p.contractId === contractId),
    [plans.data, contractId],
  );
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!planId) next["sf-plan"] = "Select an execution plan.";
    ...
    if (readiness && lastShip && readiness > lastShip) {
      next["sf-readiness"] = "Cargo readiness date cannot fall after the last shipping date.";
    }
    setErrors(next);
    ...
    const res =
      mode === "create"
        ? await api.createShipment({
            ...
            commodityReadinessDate: readiness || undefined,
            lastShippingDate: lastShip || undefined,
          })
        ...
        <form className="card card__body stack" onSubmit={submit} noValidate>
          ...
          <div className="fields">
            ...
            <FormRow
               ...
               */
              /* The link to create a plan went with the execution-plan screens on
                 8 September 2026. A contract with no plan is therefore a dead end on this
                 screen, and the hint says so rather than offering an action that no longer
                 exists — see CHANGES.md. */
              hint={
                !contractId
                  ? "Select a contract first"
                  : contractPlans.length === 0
                    ? "This contract has no execution plan, and a shipment cannot be raised without one. Execution planning is no longer a screen in the mock-up, so there is no way to add one here."
            ...
            >
              <SelectInput
                ...
                required
                error={errors["sf-plan"]}
                disabled={!contractId || mode === "edit"}
                options={contractPlans.map((p) => ({
                  value: p.id,
                  label: `${p.planningNo} — ${formatMt(p.plannedQuantityMt)} — ${p.assignedTo}`,
                }))}
              />
            </FormRow>
            ...
            <FormRow label="Cargo readiness date" htmlFor="sf-readiness" error={errors["sf-readiness"]}>
              <TextInput
                id="sf-readiness"
                type="date"
                value={readiness}
                onChange={setReadiness}
                error={errors["sf-readiness"]}
              />
            </FormRow>
            ...
            <FormRow
              label="Last shipping date"
              htmlFor="sf-lastship"
              hint={contract ? `Contract period ends ${formatDate(contract.shipmentPeriodEnd)}` : undefined}
            >
              <TextInput id="sf-lastship" type="date" value={lastShip} onChange={setLastShip} />
            </FormRow>
          </div>
