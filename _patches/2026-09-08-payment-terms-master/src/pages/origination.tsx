...
import {
  ...
  counterpartyName,
  isMasterPaymentTerm,
  /* Aliased: `DealAgreementForm` has a local state variable called `paymentTerms`, and an
     unaliased import would be shadowed by it inside exactly the component that needs it. */
  paymentTerms as paymentTermMaster,
...
function DealAgreementForm({ opportunity }: { opportunity: Opportunity }) {
  ...
  /**
   * Payment terms from the master, 8 September 2026. The extra option cannot be reached from
   * this form as it stands — the field starts blank, because a deal is agreed once — but it is
   * carried here as it is on the contract, so that any future prefill from an opportunity that
   * already holds a term (`At sight, D/P` on OPP-2026-011) shows it rather than dropping it.
   * Deleting the guard because today's path cannot reach it is how the trader drop-down came
   * to lose the fifth name on 5 September.
   */
  const paymentTermOptions = [
    ...paymentTermMaster()
      .filter((t) => t.active)
      .map((t) => ({ value: t.label, label: t.label })),
    ...(isMasterPaymentTerm(paymentTerms)
      ? []
      : [{ value: paymentTerms, label: `${paymentTerms} — not in the master` }]),
  ];
  ...
  if (!paymentTerms.trim()) errors["deal-payment"] = "Select the agreed payment terms.";
      ...
      <form className="stack" onSubmit={submit} noValidate>
        ...
        <div className="fields">
          ...
          <FormRow
            ...
            hint={`From ${paymentTermsSourceName()}, since 8 September 2026. It is the same list the contract reads, so the term agreed here is the term §6.2 activity 2 carries onto the contract — which is what the two captured deals, spelling their terms in words, could not do.`}
          >
            <SelectInput
              id="deal-payment"
              value={paymentTerms}
