...
export const seedCountryConfigs: CountryConfig[] = [
  ...
  {
    /*
     * Chad — added 9 September 2026 alongside the country context in `c2.ts`. See the note
     * there: the Export module operates in five countries and this configuration held four,
     * which became visible the moment the header's country began scoping the Export screens.
     *
     * [OPEN] Currency, season window and holidays are stated from the CEMAC zone and from
     * Ethiopia's window rather than from a source, because no Chad configuration is recorded
     * in the workspace. Business confirmation required.
     */
    code: 'TD', country: 'Chad', iso: 'TD', status: 'Active', version: 1, effectiveFrom: '2026-09-09',
    localCurrency: 'XAF', reportingCurrency: 'USD', workingDays: 'Monday to Friday',
    holidays: [{ date: '2026-08-11', name: 'Independence Day' }, { date: '2026-12-25', name: 'Christmas Day' }],
    season: '26/27', seasonStart: 'October', seasonEnd: 'March',
    defaultLanguage: 'French', additionalLanguages: ['Arabic'],
    dateFormat: 'dd/MM/yyyy', timeFormat: '24-hour',
    numberFormat: { decimal: ',', thousands: ' ', places: 0 },
    currencyDisplay: 'Code before', defaultUnit: 'MT',
    addressFormat: 'Street, district, city, country'
...
export const seedStepConfigs: StepConfig[] = (() => {
  ...
  const countries = ['Sudan', 'Ethiopia', 'Chad', 'Tanzania', 'Mozambique', 'Egypt'];
  countries.forEach((country) => {
    STEP_CATALOGUE.forEach((s) => {
      ...
      /*
       * Chad added 9 September 2026. The Export module's own country profile for Chad carries
       * `usesExportContract: true`, sourced from Ethiopia & Chad Execution.vsdx, so leaving the
       * step off here would have given two answers to one question — the configuration saying
       * the export contract does not apply in Chad while the Export screens raise one.
       *
       * [OPEN] Which of the two is right. The workshop notes say only that the Ex contract is
       * "not applicable in all countries (not applicable in Tanzania…)" and name no positive
       * list, so both readings come from the process diagrams rather than from a rule.
       */
      if (s.key === 'op-ex-contract') applies = country === 'Sudan' || country === 'Ethiopia' || country === 'Chad';
