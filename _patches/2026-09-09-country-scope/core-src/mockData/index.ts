...
/*
 * The countries COTS operates in — Chad added 9 September 2026.
 *
 * This list is what the header's country selector offers, filtered to the countries the
 * account is scoped to. It had four; the Export module has always had five. See the note on
 * `COUNTRY_CONTEXT` in `c2.ts` for why the difference stopped being harmless.
 */
export const COUNTRIES = ['Sudan', 'Ethiopia', 'Chad', 'Tanzania', 'Mozambique'];
...
export const seedUsers: User[] = [
  {
    ...
    /*
     * Widened to every operating country on 9 September 2026.
     *
     * The header's selector offers the countries an account is scoped to, and from that date
     * the country it is set to also scopes every Export list. The system administrator held
     * three of the five, so two countries' Export records — Chad's and Mozambique's — were
     * unreachable from the only account that can see every module. The other demo accounts
     * keep their narrow scopes deliberately: a single-country account is what makes the
     * header's selector read-only, which is the C1 rule the shell demonstrates (Step 2).
     */
    countryScope: ['Sudan', 'Ethiopia', 'Chad', 'Tanzania', 'Mozambique'],
    moduleScope: MODULES,
    assignments: [
      { id: 'A-001', role: 'SYSTEM_ADMINISTRATOR', countryScope: ['Sudan', 'Ethiopia', 'Chad', 'Tanzania', 'Mozambique'], moduleScope: MODULES, source: 'Manual', from: '2025-01-01', status: 'Active' }
    ],
