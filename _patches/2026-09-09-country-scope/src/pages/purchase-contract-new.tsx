...
import { COUNTRY_PROFILES, countryName } from "../domain/variants";
import { activeCountryScope, api } from "../services/store";
...
export function PurchaseContractForm() {
  ...
  /**
   * The origin is the country the session is working in — 9 September 2026.
   *
   * A contract is raised in the country the person is in, and from that date the country is
   * a property of the session rather than a question on a form: it is the one in the header,
   * and every list this contract will appear in is filtered to it. Asking for it here would
   * make a second answer possible, and a contract raised with the wrong one would be
   * invisible to the person who raised it. `createContract` refuses the mismatch as well.
   *
   * Unscoped — no country on the session — the drop-down is offered as before, because then
            ...
            <div className="pcform__grid">
              ...
              {scope ? (
                <FormRow
                  label="Origin"
                  htmlFor="pc-origin"
                  hint={`Read from the session, not asked for. The contract is raised in ${countryName(scope)}, which is the country in the header; every list it appears in is filtered to that country. Change the country in the header to raise a contract elsewhere.`}
                >
                  <div id="pc-origin" data-testid="pc-origin-fixed">
                    <strong>{countryName(scope)}</strong>
                  </div>
                </FormRow>
              ) : (
                <FormRow
                  ...
                  hint="The session names no country, so the origin is asked for here."
                >
                  <SelectInput
                    id="pc-origin"
                    value={draft.origin}
