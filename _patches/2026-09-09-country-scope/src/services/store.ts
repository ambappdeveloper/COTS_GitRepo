...
export const api = {
  ...
  async setShipmentBankDetails(
    ...
    if (draft.bankBranch && !branches.includes(draft.bankBranch)) {
      return delay(failResult(`${bank.name} has no branch called “${draft.bankBranch}” in master data.`));
    }
    ...
    pushAudit(id, {
      ...
      action: "Updated",
      entity: `Shipment ${s.shipmentNo}`,
      note: `Bank details recorded: ${bank.name}${draft.bankBranch ? ` · ${draft.bankBranch}` : ""}.`,
    });
    notify();
    return delay(okResult(clone(s)));
  },
