...
import { api, resetStore, resolvedMilestonesFor, setLatency } from "../services/store";
import {
...
describe("the tags specification takes an attachment", () => {
  /* "Attachment should be added under Tag specification section." — 8 September 2026. */

  it("offers the field inside the tags specification section, not elsewhere on the tab", async () => {
    const { container, unmount } = renderRoute("/contracts/:id/:tab", "/contracts/ct-1/quality", <ContractDetail />);
    try {
      ...
      expect(field).toBeTruthy();
      /* Inside the tags section and after the batch-code control, so it saves with the
         specification rather than with the quality terms above it. */
      const section = container.querySelector("#tg-batch")?.closest("section, div.card, details");
      const batch = container.querySelector("#tg-batch");
      expect(section ?? batch).toBeTruthy();
      expect(
        batch?.compareDocumentPosition(field!) ?? 0 & Node.DOCUMENT_POSITION_FOLLOWING,
      ).toBeTruthy();
