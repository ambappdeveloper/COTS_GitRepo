...
export const CONTRACT_LIFECYCLE: StepperStep[] = [
  ...
  /* `execution_planned` (P4) removed 9 September 2026 — see the note on SHIPMENT_LIFECYCLE. */
...
export const SHIPMENT_LIFECYCLE: StepperStep[] = [
  /* `planned` (P4) was the first step and rolled up `execution_plan_created`. Both went on
     9 September 2026 with execution planning: a step whose only source no longer exists shows
     as not-applicable for ever, which reads as a gap in the process rather than as a step that
     was retired. Nine steps, not ten. */
...
export const EXECUTION_FLOW: MilestoneDefinition[] = [
  ...
  /* ---- Band: Planning ---- */
  /*
   * `execution_plan_created` stood here until 9 September 2026. It was removed with the record it
   * named: execution planning went on 8 September on the grounds that split shipments replace it,
   * and a milestone that reports the creation of a record the system no longer has is a step
   * nobody can ever complete. `stock_allocated` now waits on the contract instead, which is what
   * the plan waited on.
   */
  {
    /*
     * Bank details on the split — 9 September 2026.
     *
     * "There should be notification to finance once split so they enter bank details (bank name,
     * branch name)." This *is* that notification. Export keeps no task queue of its own — see
     * `src/integration/exportTasks.ts` — so the way to put work in front of a named function is
     * to make it a milestone they own. Raising a Sudan split therefore puts a row in the one
     * Actions Inbox, addressed to Finance, and the row clears when the details are recorded.
     *
     * It is `ready` as soon as the contract is issued rather than waiting on anything upstream,
     * because the bank is a fact about the split and not about its cargo: Finance can answer it
     * the moment the split exists, which is when they are asked.
     *
     * Nothing is blocked by its absence. No source makes the bank a gate, and the mock-up's
     * standing rule (decision D-10) is that an unstated gate is not invented.
     */
    key: "shipment_bank_details",
    name: "Bank and branch recorded for the split",
    phase: "P13",
    band: "Planning",
    owner: "finance",
    prerequisites: ["contract_issued"],
    appliesTo: (ctx) => ctx.country.requiresShipmentBankDetails,
    evidence:
      "Business instruction, 9 September 2026: bank details requested per split, Sudan only, with Finance notified on the split",
  },
  {
    key: "stock_allocated",
  ...
  },
  {
    ...
    /* Waited on `execution_plan_created` until 9 September 2026. A prerequisite naming a
       milestone the flow no longer defines is never met, so this step would have sat at
       not-started for ever — the export contract is raised against the contract now, and waits
       on the contract. */
    prerequisites: ["contract_issued"],
