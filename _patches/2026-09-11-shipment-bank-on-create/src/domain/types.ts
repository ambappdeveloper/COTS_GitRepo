...
export interface CountryProfile {
  ...
  /**
   * Whether Finance is asked for the bank and branch on every split shipment — 9 September 2026,
   * "Request bank details only for Sudan". See the note on the Sudan profile.
   */
  requiresShipmentBankDetails: boolean;
  /** Chad and Ethiopia have an inland leg before the load port. */
  hasInlandTransitLeg: boolean;
  inlandLegLabel?: string;
  /**
   * The second country the cargo passes through, where there is one — 9 September 2026.
   *
   * "If country is Sudan user can add price for Sudan only; for other countries user can add
   * price between one or two countries." The one-or-two is the route: Sudan loads at Port Sudan
   * and never leaves, so it is priced once; Chad goes overland into Cameroon for Douala and
   * Ethiopia into Djibouti, so those are priced in each. A name and not a `CountryUnit` — these
   * are transit countries, not operating units, and the unit list must not grow to hold them.
   */
...
/**
 * A price Finance records against one country of the route — 9 September 2026.
 *
 * "If country is Sudan user can add price for Sudan only; for other countries user can add price
 * between one or two countries. This should be entered by finance."
 *
 * The countries are derived from the route rather than chosen: the contract's origin, plus the
 * transit country where the profile names one. So a Sudan contract offers one row and a Chad or
 * Ethiopian contract offers two. Nothing here decides *what* the price is — no source says
 * whether it is a cost, a transfer price or a quoted rate — so the field is labelled with the
 * country and left for the business to define. Recorded as [OPEN] on the screen.
 */
export interface ReviewFeedbackPrice {
  /** `origin`, or `transit` for the second leg. */
  leg: "origin" | "transit";
  /** How the country reads on screen — the profile's own name, or its `transitCountry`. */
  countryName: string;
  amount: Money;
}
...
export interface ReviewFeedback {
  ...
  comment?: string;
  /**
   * Prices per country of the route — Finance's row only, added 9 September 2026.
   *
   * On the type rather than on a Finance-shaped subtype because `reviewFeedback` is one list the
   * screen walks; a second shape would need the walk to branch. Every other function's row
   * leaves it undefined, and the dialog only offers it when the row is Finance's.
   */
  prices?: ReviewFeedbackPrice[];
}
...
export interface CargoReadiness {
  id: string;
  /**
   * The shipment whose cargo this describes.
   *
   * It was `executionPlanId` until 8 September 2026, when execution planning was removed on the
   * grounds that split shipments replace it. Readiness had no other parent — no contract, no
   * shipment — so unlike `Shipment`, `ExportContract` and `AdvancePayment`, which all already
   * carried `contractId`, this one had to be re-pointed rather than simply shortened. The
   * shipment is the parent that keeps what the plan gave: a contract shipped in four splits has
   * four readiness records, not one covering all of them.
   */
  shipmentId: string;
...
/**
 * The bank a split shipment's documents and proceeds go through — 9 September 2026.
 *
 * "After split shipment ask bank details for each split … bank name, branch name."
 *
 * WHY IT IS PER SHIPMENT AND NOT PER CONTRACT. The instruction is explicit — *for each split* —
 * and it is the point of the change: one contract shipped in four splits may route them through
 * four different banks, which a single field on the contract cannot say.
 *
 * WHERE IT WAS BEFORE. The execution plan carried `bank` and `bankBranch` until 8 September 2026,
 * when planning was removed. Those values are not lost: the captured plans' banks are re-seeded
 * onto the shipments each plan carried.
 *
 * WHO FILLS IT IN. Finance, and not whoever raises the shipment — which is why creating a Sudan
 * split raises the `shipment_bank_details` milestone against them.
 *
 * AMENDED 11 SEPTEMBER 2026. The New shipment form now offers the bank and branch as **optional**
 * fields, on a business instruction. Finance is still who is asked when they are left blank, and
 * the milestone still does the asking — what changed is that a raiser who already knows the bank
 * may record it with the split instead of leaving a task for a fact they were holding. The record
 * is unchanged; only the number of ways into it. `providedBy` therefore names Dubai Execution on
 * a split recorded at creation and Finance on one answered from the Summary card, which is the
 * only way to tell the two apart after the event.
 */
export interface ShipmentBankDetails {
  /** The counterparty master's bank record — a governed list, not a typed name. */
  bankId: string;
  /** The name as it read when it was recorded, so a renamed master entry does not rewrite history. */
  bankName: string;
  /** From the bank's own branch list. Optional: the master holds no branch for some banks. */
  bankBranch?: string;
  providedOn?: IsoDate;
  providedBy?: string;
}
...
export interface Shipment {
  ...
  contractId: string;
  exportContractId?: string;
  /** Asked for only where the country profile requires it — Sudan today. */
  bankDetails?: ShipmentBankDetails;
 ...
 */
/**
 * The tag specification's state — remodelled 9 September 2026.
 *
 * "Origin execution will send tag and Dubai execution will confirm or add amendment; there is a
 * workflow."
 *
 * It was `not_started → specified → agreed → reissued`, a single line with no second party in
 * it: one screen, one form, and nothing recording who had said what. The workflow the business
 * describes has two parties and a return leg, so the states name the *hand-off* rather than the
 * document's tidiness:
 *
 ...
 *
 *   · `sent`      — Origin Execution has sent the tag and it is with Dubai Execution.
 *   · `amended`   — Dubai returned it with an amendment; it is back with Origin.
 *   · `confirmed` — Dubai accepted it. Not terminal: reprocessing re-opens it (§6.5 exception).
 *   · `reissued`  — reprocessing changed the packaging and the tags, so Origin sends again.
 *
 * `specified` and `agreed` were renamed rather than kept beside the new values: no captured
 * record carries either, so there is no history to preserve and two vocabularies for one state
 * would be the worse outcome.
 */
export type TagSpecificationState = "not_started" | "sent" | "amended" | "confirmed" | "reissued";

/** What happened, and who did it. One entry per hand-off. */
export type TagSpecificationAction = "sent" | "amended" | "confirmed" | "reissued";

/**
 * One leg of the exchange between Origin Execution and Dubai Execution.
 *
 * A single note field would hold only the newest amendment, and a tag amended twice for
 * different reasons is exactly the case that matters — the second reviewer needs to see what the
 * first one asked for. So every send, amendment and confirmation is an entry, and the screen
 * shows the thread.
 */
export interface TagSpecificationEvent {
  id: string;
  action: TagSpecificationAction;
  /** The function that acted — `partner_execution` sends, `dubai_execution` answers. */
  party: Role;
  /** The person, read from the signed-in session rather than typed. */
  by?: string;
  at: IsoDate;
  /** What the amendment asks for, or what the sender wants noted. Required on an amendment. */
  note?: string;
  /** The file this leg carried, where it carried one. */
...
export interface TagSpecification {
  ...
  /**
   * The tag or artwork file the specification is written against — added 8 September 2026.
   *
   * A file *name* only, as `Contract.artworkDesignFileName` is: this prototype stores names and
   * not files, and says so on the field.
   *
   * [OPEN] One attachment or several. A specification can be re-issued after reprocessing —
   * `reissuedAfterReprocessingOn` exists for exactly that — and a single field is overwritten
   * when it is, losing the artwork the buyer originally agreed. Modelled as one because the
   * instruction says "attachment", and because inventing a version history would assert a rule
   * no source states. It becomes a collection the day the business says the earlier file is
   * kept.
   */
  attachmentFileName?: string;
  /**
   * Every leg of the exchange, oldest first — added 9 September 2026 with the two-party workflow.
   * The current `state` is derivable from the last entry; both are stored, because the state is
   * what every other screen reads and re-deriving it on each render would invite the two to
   * disagree.
   */
  exchange: TagSpecificationEvent[];
  note?: string;
}
