...
import { CountryScopeNotice } from "../components/CountryScopeNotice";
import { ExecutionFlow, LifecycleStepper } from "../components/status-flow";
import { DataTable, type Column } from "../components/table";
import { ErrorSummary, FormActions, FormRow, RequiredLegend, SelectInput, TextInput } from "../components/form";
...
export function ShipmentDetail() {
  ...
  return (
    <>
      ...
      <div className="page">
        {/*
          9 September 2026 — a shipment opened by link from another country's session says so.
          Above the risk banner because it changes how everything below it should be read:
          the contract, readiness and transport panels are filtered to the session's country
          and will be empty for this record.
        */}
        <CountryScopeNotice recordCountry={s.country} what={`Shipment ${s.shipmentNo}`} />
        ...
        {activeTab === "summary" ? (
          <>
            ...
            <ShipmentBankDetailsCard s={s} onSaved={() => shipment.reload()} />
...
/**
 * Bank details for one split — 9 September 2026.
 *
 ...
 * branch name)."
 *
 * WHY IT IS HERE. Finance answers this, and Finance does not raise the split — Dubai Execution
 * does. So the split may be raised without the details, the `shipment_bank_details` milestone
 * leaves a row in the Actions Inbox addressed to Finance, and the row links here.
 *
 * AMENDED 11 SEPTEMBER 2026 — the New shipment form asks too. The original of this comment said
 * the fields belonged here "and not on the create form", on the reasoning above. A business
 * instruction of 11 September puts them on the create form as well, and the reasoning survives
 * rather than being overturned: the fields there are **optional**, so the default path is still
 * the one this card was built for. What changed is that a raiser who already knows the bank is
 * no longer required to leave a task for a fact they were holding.
 *
 * Both routes write through `resolveShipmentBankDetails` in the service layer, so the create
 * form cannot file a value this card would refuse, or the reverse. This card remains the only
 * way to **change** a recorded bank: the edit form does not carry the fields.
 *
 * WHY IT IS ON THE SUMMARY. It is captured at the start of the shipment's life and read for the
 * rest of it — the bank submittal at Phase 14 is the eventual consumer, but a card that only
 * appeared there would be invisible at the moment Finance is asked.
 *
 * COUNTRY. The card renders everywhere and says something different where the profile does not
 * require the details, rather than disappearing: a reviewer comparing a Sudan shipment with a
 * Tanzanian one should be able to see *why* one asks and the other does not.
 */
function ShipmentBankDetailsCard({ s, onSaved }: { s: Shipment; onSaved: () => void }) {
  const toast = useToast();
  const { user } = useAuth();
  const profile = COUNTRY_PROFILES[s.country];
  const required = profile?.requiresShipmentBankDetails ?? false;
  const held = s.bankDetails;

  const banks = counterpartiesOfType("bank");
  const [bankId, setBankId] = useState(held?.bankId ?? "");
  const [branch, setBranch] = useState(held?.bankBranch ?? "");
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  /**
   * Whether the form is showing.
   *
   * The first version of this card rendered the form only while the details were missing, which
   * meant that on every split that had them there was no way to add or change anything — a bank
   * recorded wrongly was recorded for good, and a reviewer opening a seeded split saw values and
   * no control at all. Outstanding opens the form; recorded shows the values with a Change
   * action beside them.
   */
  const [editing, setEditing] = useState(false);

  /* The branch list belongs to the bank, so changing the bank drops a branch the new one does
     not have — the dependent-master rule the commodity type and the bank branch already use. */
  ...
  async function save() {
    setError(null);
    if (!bankId) {
      setError("Select the bank.");
      return;
    }
    setSaving(true);
    const res = await api.setShipmentBankDetails(s.id, {
      bankId,
      bankBranch: branch || undefined,
      providedBy: user?.displayName,
    });
    setSaving(false);
    if (!res.ok) {
      setError(res.reason);
    ...
    toast.push("ok", `Bank details recorded on ${s.shipmentNo}.`);
    setEditing(false);
    onSaved();
  }
  ...
  if (!required) {
    return (
      <SummaryCard title="Bank details">
        <p className="small muted">
          Not requested in {profile?.name ?? "this country"}. The bank and branch are asked for on every
          split only where the country profile requires them — Sudan today. Every captured execution plan
          named a bank, including the ones outside Sudan, so whether the requirement is really Sudan-only
  ...
  return (
    <SummaryCard title="Bank details" tone={held ? "ok" : "warn"}>
      {held && !editing ? (
        <>
          <FieldGrid
            columns={1}
            fields={[
              { label: "Bank", value: held.bankName },
              { label: "Branch", value: held.bankBranch },
              { label: "Recorded on", value: formatDate(held.providedOn) },
              { label: "Recorded by", value: held.providedBy },
            ]}
          />
          <p className="xsmall muted" style={{ marginTop: "0.5rem" }}>
            Recorded against this split. A contract shipped in several splits may route each through a
            different bank, which is why the detail is held here and not on the contract.
          </p>
          <FormActions>
            <button
              type="button"
        ...
        <>
          {held ? (
            <p className="small">
              Changing the bank recorded on {s.shipmentNo}. The previous value is replaced — this
              prototype keeps no history of it, which is a question for the business rather than a
              decision taken here.
            </p>
          ) : (
            <p className="small">
              Outstanding. Raising this split put a task in the Actions Inbox for Finance — nothing is
              blocked while it is unanswered.
            </p>
          )}
          {error ? (
          ...
          <FormRow label="Bank" htmlFor="sb-bank" required hint="From the counterparty master.">
            <SelectInput
              id="sb-bank"
              value={bankId}
              onChange={chooseBank}
            ...
            hint={
              !bankId
                ...
                : branches.length === 0
                  ...
                  : "The branches held for this bank"
            }
          >
            <SelectInput
              id="sb-branch"
          ...
          <FormActions>
            <button type="button" className="btn btn--primary" onClick={() => void save()} disabled={saving}>
              {saving ? "Saving…" : held ? "Save bank details" : "Record bank details"}
            </button>
            {held ? (
              <button
                type="button"
...
export function ShipmentForm({ mode }: { mode: "create" | "edit" }) {
  ...
  /**
   * The bank and its branch, asked for on the split itself — 11 September 2026.
   *
   * Create only. The edit form does not carry them: the shipment's own Summary already has a
   * Bank details card with a Change action, and two editors for one field is how a screen and a
   * card come to disagree about what was recorded.
   */
  ...
  /**
   * Whether this split is asked for a bank at all.
   *
   * Read from the **contract's origin**, not from the session's country: the origin is what the
   * new shipment is about to be given (`createShipment` sets `country: contract.origin`), and it
   * is the country whose profile the service layer will check on save. Reading the session here
   * would let the form ask for a bank the save then refuses.
   *
   * Sudan today. The requirement is configuration — `requiresShipmentBankDetails` on the country
   * profile — so a second country that needs it is a profile change and not a change here.
   */
  const bankCountry = contract ? COUNTRY_PROFILES[contract.origin] : undefined;
  const asksForBank = mode === "create" && (bankCountry?.requiresShipmentBankDetails ?? false);
  const bankOptions = counterpartiesOfType("bank");
  const bankBranches = bankBranchesFor(bankId);

  /* The branch belongs to the bank, so changing the bank drops a branch the new one does not
     have — the dependent-master rule the container type and the Summary card both follow. */
  function chooseBank(next: string) {
    setBankId(next);
  ...
  async function submit(e: React.FormEvent) {
    ...
    /* The bank is optional — leaving it blank is the 9 September behaviour, and the split is
       raised with the Finance task outstanding. A branch without a bank is not a partial answer
       though: the branch list is keyed by the bank, so a branch alone names nothing. The service
       layer refuses the same combination. */
    if (asksForBank && bankBranch && !bankId) {
      next["sf-bank"] = "Select the bank, or clear the branch.";
    }
    ...
    const res =
      mode === "create"
        ? await api.createShipment({
            ...
            /* Sent only where the country asks for them. Guarded here as well as hidden,
               because the contract can be changed after a bank is picked and a value the
               screen no longer shows must not be filed behind the user's back. */
            bankId: asksForBank ? bankId || undefined : undefined,
            bankBranch: asksForBank ? bankBranch || undefined : undefined,
            recordedBy: asksForBank && bankId ? user?.displayName : undefined,
          })
      ...
      <div className="page">
        <form className="card card__body stack" onSubmit={submit} noValidate>
          ...
          <div className="fields">
            ...
            {/*
              Bank details on the split — 11 September 2026.

              Optional, and deliberately so. The 9 September instruction put these in front of
              Finance through the Actions Inbox, because Finance answers them and Dubai Execution
              raises the split. That has not been withdrawn: leaving both blank raises the split
              exactly as before, with the task outstanding. What is new is that the raiser who
              already knows the bank can say so here rather than leaving a task to be answered
              with a fact they were holding all along.

              Shown only where the contract's origin requires them — Sudan today.
            */}
            {asksForBank ? (
              <>
                <FormRow
                  label="Bank"
                  htmlFor="sf-bank"
                  error={errors["sf-bank"]}
                  hint={`From the counterparty master. Optional — leave blank and ${bankCountry?.name ?? "this country"} raises the split with the bank outstanding for Finance.`}
                >
                  <SelectInput
                    id="sf-bank"
                    value={bankId}
                    onChange={chooseBank}
                    error={errors["sf-bank"]}
                    placeholder="Leave for Finance"
                    options={bankOptions.map((b) => ({ value: b.id, label: b.name }))}
                  />
                </FormRow>

                <FormRow
                  label="Bank branch"
                  htmlFor="sf-bankbranch"
                ...
                >
                  <SelectInput
                    ...
                    disabled={!bankId || bankBranches.length === 0}
                    options={bankBranches.map((b) => ({ value: b, label: b }))}
                  />
                </FormRow>
              </>
            ) : null}
          ...
          {asksForBank && !bankId ? (
            <Banner tone="info" title="The bank can be left to Finance">
              Saving without one raises this split with{" "}
              <strong>Bank and branch recorded for the split</strong> outstanding, which puts a row in
              the Actions Inbox addressed to Finance. Nothing is blocked by its absence, and it can be
              recorded later from the shipment's own Bank details card.
            </Banner>
          ) : null}
