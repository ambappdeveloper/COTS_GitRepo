...
export function ContractDetail() {
  ...
  return (
    <>
      ...
      <div className="page">
        {/*
          9 September 2026 — the record opens whoever follows the link, and says so when it
          belongs to a country other than the one the session is working in. Above the tabs'
          content so it is read before anything on the page, including the shipment and
          readiness panels, which are themselves filtered to the session's country and will
          be empty here.
        */}
        <CountryScopeNotice recordCountry={c.origin} what={`Contract ${c.contractNo}`} />
        ...
        {activeTab === "export-contract" ? (
          <>
            {/*
              New request, raised from the contract's own tab — 13 September 2026.
              "Add a button new request for Export contract related to the Purchase Contract, in
               the new Export Contract screen form, it will inherit the Purchase Contract no."

              The action existed only on the Pre-clearance list header, where it opens a screen
              that asks which contract the request is for. Reached from here the contract is
              already settled, so it is carried in the query string and the screen states it
              rather than asking again.

              WHERE IT IS NOT OFFERED. Tanzania and Mozambique do not use an export contract, so
              the button is absent there and the empty state below says why. An action that leads
              to a screen which refuses the request is worse than no action — the rule the
              pre-clearance record's own issuance action already follows.
            */}
            <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
              ...
              {COUNTRY_PROFILES[c.origin].usesExportContract ? (
                <Link className="btn btn--primary btn--sm" to={`/pre-clearance/new?contract=${c.id}`}>
                  New request
                </Link>
              ) : null}
            </div>
            {ctExport.length === 0 ? (
              <div className="card">
                <EmptyState
                  title="No export contract"
                  action={
                    COUNTRY_PROFILES[c.origin].usesExportContract ? (
                      /* Also here, so an empty tab is not a dead end. */
                      <Link className="btn btn--primary" to={`/pre-clearance/new?contract=${c.id}`}>
                        New request
                      </Link>
                    ) : undefined
                  }
